...
-- | Pretty-printer for Asmlang.

module Asmlang.Print where
...
instance Print Asmlang.Abs.Ident where
  prt _ (Asmlang.Abs.Ident i) = doc $ showString i
instance Print Asmlang.Abs.Pgm where
  prt i = \case
    Asmlang.Abs.PDefs primitives -> prPrec i 0 (concatD [prt 0 primitives])

instance Print [Asmlang.Abs.Primitive] where
  prt _ [] = concatD []
  prt _ (x:xs) = concatD [prt 0 x, prt 0 xs]

instance Print Asmlang.Abs.Primitive where
  prt i = \case
    Asmlang.Abs.PInst instruction -> prPrec i 0 (concatD [prt 0 instruction])
    Asmlang.Abs.PLabel label -> prPrec i 0 (concatD [prt 0 label, doc (showString ":")])

instance Print Asmlang.Abs.Label where
  prt i = \case
    Asmlang.Abs.Llabel id_ -> prPrec i 0 (concatD [prt 0 id_, doc (showString ":")])

instance Print Asmlang.Abs.Instruction where
  prt i = \case
    Asmlang.Abs.Inop -> prPrec i 0 (concatD [doc (showString "nop")])
